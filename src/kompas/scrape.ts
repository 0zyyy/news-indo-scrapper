...
export default class KompasScrapper {
    ...
    async getUrl(query: string, date?: string): Promise<cheerio.CheerioAPI>{
        const resp =  await axiosClient.get(this.host + `site=${query}&date=${date ?? ""}`);
        ...
        try{
            ...
            articleList.each((_, element) => {
                this.articles.push({
                    ...
                    article_url: page(element).find("a.article-link").attr("href") ?? "No link provided",
                });
            });
    ...
    async getContentArticle(article_url: string) {
        try{
            const resp = await axiosClient.get(article_url, {
                headers: {
            ...
            const regex = /var keywordBrandSafety = "([^"]*)";/;
            const match = script.html()!.match(regex);

            // Extracted variable value
            const extractedVariable = match ? match[1] : null;

            console.log(extractedVariable);
        }catch(e) {
            console.log(e);
...
// ng.getArticles("nasional").then((resp) => console.log(resp));
ng.getContentArticle("https://nasional.kompas.com/read/2024/03/10/13461681/raih-suara-terbanyak-dalam-pileg-dpd-fahira-idris-terima-kasih-warga-jakarta")